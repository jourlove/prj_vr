...
if(!in_array($act,array_keys($act_legal))){
  ...
  //不是注册和找回密码时，自动提取手机号
     ...
     $phone = $Db->query("select phone from ".$Base->table('user')." where pk_user_main = ".$user['pk_user_main']."","One");
  }
  //图片验证码
  ...
  if($_SESSION['captcha'][$act]!=md5(strtolower($captcha))){
   ...
   //1个手机号60秒内只能发送一次
    ...
    //阿里云通信发送短信
    if($_lang['global_sms']=='aliyuntongxun'){
      require './source/aliyuntongxun/api_demo/alicom-dysms-api/smsDemo.php';
      sendSMS($_lang['aliyuntongxun_config']['AccessKeyID'],$_lang['aliyuntongxun_config']['AccessKeySecret'],$phone,$_lang['aliyuntongxun_config']['freesignname'],$_lang['aliyuntongxun_config']['templateId'],$sms_captcha); 
    }
