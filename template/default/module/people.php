...
function listMembers($level,$filter,$region,$page,$size,$maxNode){
	$sql = 'SELECT u.level,u.nickname,u.phone,u.pk_user_main,p.avatar,w.thumb_path,w.view_uuid,w.praised_num,w.name,COUNT(w.pk_works_main) AS total , SUM(w.browsing_num) AS popular FROM '.$GLOBALS['Base']->table('user').' u LEFT JOIN '.$GLOBALS['Base']->table('user_profile').' p ON p.pk_user_main  = u.pk_user_main LEFT JOIN '.$GLOBALS['Base']->table('worksmain').' w ON u.pk_user_main = w.pk_user_main WHERE 1=1 ';
	if ($level>0) {
