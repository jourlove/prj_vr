...
var openMediaResObj;
var mapModalEl;
var hotSpotModalTitle = ["添加全景切换热点","添加超链接热点","添加图册","添加文本热点","添加音乐","添加图文热点","添加分组","添加环物热点","添加视频热点","添加红包"];
var hotSpotDataKey = ["scene","link","image","text","voice","imgtext","obj",'video','redpack'];
var _user_view_uuid;
var hotspotIdx = 0;
...
var paster_img_w_Slider;
var paster_img_h_Slider;
var paster_img_rx_Slider;
var paster_img_ry_Slider;
var paster_img_rz_Slider;
var paster_img_d_Slider;
...
$(function () {
    ...
    $("#imgUpload").fileinput({
        ...
        if(mediaType && mediaType == '1'){
            ...
            if (!checkFileAlowedExtensions(["jpg", "png", "gif"], files[0])){
    ...
    ex_w_Slider = $("#ex_w").slider({
        tooltip: 'hide'
    });
    ex_h_Slider = $("#ex_h").slider({
        tooltip: 'hide'
    });
    ex_rx_Slider = $("#ex_rx").slider({
        tooltip: 'hide'
    });
    ex_ry_Slider = $("#ex_ry").slider({
        tooltip: 'hide'
    });
    ex_rz_Slider = $("#ex_rz").slider({
        tooltip: 'hide'
    });
    ex_d_Slider = $("#ex_d").slider({
        tooltip: 'hide'
    });
    ...
    ex_w_Slider.on("slide", function (data) {
        var krpano=getKrpano();
		$('#ex_w_txt').val(data.value);
        set_hotspot_video_tie(krpano,hotspotName,"width",data.value);
    });
	$('#ex_w_txt').bind('input propertychange', function(){
		var krpano=getKrpano();
		var val = $('#ex_w_txt').val();
		if(val<100) val = 100;
		if(val>2000) val = 2000;
		ex_w_Slider.slider("setValue", parseInt(val));
		set_hotspot_video_tie(krpano,hotspotName,"width",Math.floor(val*100)/100);
        clearTimeout(g_tmo);
        g_tmo = setTimeout(function(){
            update_video_tie_value(getCurrScene(),hotspotName,{'width':val});
        },500);
	});
    ex_w_Slider.on("slideStop", function (data) {
	   $('#ex_w_txt').val(data.value);
        update_video_tie_value(getCurrScene(),hotspotName,{'width':data.value});
    });

     ex_h_Slider.on("slide", function (data) {
         var krpano=getKrpano();
		 $('#ex_h_txt').val(data.value);
        set_hotspot_video_tie(krpano,hotspotName,"height",data.value);
    });
	$('#ex_h_txt').bind('input propertychange', function(){
		var krpano=getKrpano();
		var val = $('#ex_h_txt').val();
		if(val<100) val = 100;
		if(val>2000) val = 2000;
		ex_h_Slider.slider("setValue", parseInt(val));
		set_hotspot_video_tie(krpano,hotspotName,"height",Math.floor(val*100)/100);
        clearTimeout(g_tmo);
        g_tmo = setTimeout(function(){
            update_video_tie_value(getCurrScene(),hotspotName,{'height':val});
        },500);
	});
    ex_h_Slider.on("slideStop", function (data) {
	   $('#ex_h_txt').val(data.value);
       update_video_tie_value(getCurrScene(),hotspotName,{'height':data.value});
    });

    ex_rx_Slider.on("slide", function (data) {
        var krpano=getKrpano();
		$('#ex_rx_txt').val(data.value);
        set_hotspot_video_tie(krpano,hotspotName,"rx",data.value);
    });
	$('#ex_rx_txt').bind('input propertychange', function(){
		var krpano=getKrpano();
		var val = $('#ex_rx_txt').val();
		if(val<-180) val = -180;
		if(val>180) val = 180;
		ex_rx_Slider.slider("setValue", parseInt(val));
		set_hotspot_video_tie(krpano,hotspotName,"rx",Math.floor(val*100)/100);
        clearTimeout(g_tmo);
        g_tmo = setTimeout(function(){
            update_video_tie_value(getCurrScene(),hotspotName,{'rx':val});
        },500);
	});
    ex_rx_Slider.on("slideStop", function (data) {
	   $('#ex_rx_txt').val(data.value);
       update_video_tie_value(getCurrScene(),hotspotName,{'rx':data.value});
    });

    ex_ry_Slider.on("slide", function (data) {
        var krpano=getKrpano();
		$('#ex_ry_txt').val(data.value);
        set_hotspot_video_tie(krpano,hotspotName,"ry",data.value);
    });
	$('#ex_ry_txt').bind('input propertychange', function(){
		var krpano=getKrpano();
		var val = $('#ex_ry_txt').val();
		if(val<-180) val = -180;
		if(val>180) val = 180;
		ex_ry_Slider.slider("setValue", parseInt(val));
		set_hotspot_video_tie(krpano,hotspotName,"ry",Math.floor(val*100)/100);
        clearTimeout(g_tmo);
        g_tmo = setTimeout(function(){
            update_video_tie_value(getCurrScene(),hotspotName,{'ry':val});
        },500);
	});
    ex_ry_Slider.on("slideStop", function (data) {
	   $('#ex_ry_txt').val(data.value);
       update_video_tie_value(getCurrScene(),hotspotName,{'ry':data.value});
    });

    ex_rz_Slider.on("slide", function (data) {
         var krpano=getKrpano();
		$('#ex_rz_txt').val(data.value); 
        set_hotspot_video_tie(krpano,hotspotName,"rz",data.value);
    });
	$('#ex_rz_txt').bind('input propertychange', function(){
		var krpano=getKrpano();
		var val = $('#ex_rz_txt').val();
		if(val<-180) val = -180;
		if(val>180) val = 180;
		ex_rz_Slider.slider("setValue", parseInt(val));
		set_hotspot_video_tie(krpano,hotspotName,"rz",Math.floor(val*100)/100);
        clearTimeout(g_tmo);
        g_tmo = setTimeout(function(){
            update_video_tie_value(getCurrScene(),hotspotName,{'rz':val});
        },500);
	});
    ex_rz_Slider.on("slideStop", function (data) {
	   $('#ex_rz_txt').val(data.value); 
       update_video_tie_value(getCurrScene(),hotspotName,{'rz':data.value});
    });
	
    ex_d_Slider.on("slide", function (data) {
        var krpano=getKrpano();
		$('#ex_d_txt').val(data.value); 
        set_hotspot_video_tie(krpano,hotspotName,"depth",data.value);
    });
	$('#ex_d_txt').bind('input propertychange', function(){
		var krpano=getKrpano();
		var val = $('#ex_d_txt').val();
		if(val<200) val = 200;
		if(val>20000) val = 20000;
		ex_d_Slider.slider("setValue", parseInt(val));
		set_hotspot_video_tie(krpano,hotspotName,"depth",val);
        clearTimeout(g_tmo);
        g_tmo = setTimeout(function(){
            update_video_tie_value(getCurrScene(),hotspotName,{'depth':val});
        },500);
	});
    ex_d_Slider.on("slideStop", function (data) {
	   $('#ex_d_txt').val(data.value);
       update_video_tie_value(getCurrScene(),hotspotName,{'depth': data.value});
    });

    paster_img_w_Slider = $("#paster_img_w").slider({
        tooltip: 'hide'
    });
    paster_img_h_Slider = $("#paster_img_h").slider({
        tooltip: 'hide'
    });
    paster_img_rx_Slider = $("#paster_img_rx").slider({
        tooltip: 'hide'
    });
    paster_img_ry_Slider = $("#paster_img_ry").slider({
        tooltip: 'hide'
    });
    paster_img_rz_Slider = $("#paster_img_rz").slider({
        tooltip: 'hide'
    });
    paster_img_d_Slider = $("#paster_img_d").slider({
        tooltip: 'hide'
    });

    paster_img_w_Slider.on("slide", function (data) {
        var krpano=getKrpano();
		$('#paster_img_w_txt').val(data.value);
        set_hotspot_video_tie(krpano,hotspotName,"width",data.value);
    });
	$('#paster_img_w_txt').bind('input propertychange', function(){
		var krpano=getKrpano();
		var val = $('#paster_img_w_txt').val();
		if(val<100) val = 100;
		if(val>2000) val = 2000;
		paster_img_w_Slider.slider("setValue", parseInt(val));
		set_hotspot_video_tie(krpano,hotspotName,"width",Math.floor(val*100)/100);
	});
    paster_img_w_Slider.on("slideStop", function (data) {
	   $('#paster_img_w').val(data.value);
       update_video_tie_value(getCurrScene(),hotspotName,{'width':data.value});
    });

    paster_img_h_Slider.on("slide", function (data) {
         var krpano=getKrpano();
		 $('#paster_img_h_txt').val(data.value);
        set_hotspot_video_tie(krpano,hotspotName,"height",data.value);
    });
	$('#paster_img_h_txt').bind('input propertychange', function(){
		var krpano=getKrpano();
		var val = $('#paster_img_h_txt').val();
		if(val<100) val = 100;
		if(val>2000) val = 2000;
		paster_img_h_Slider.slider("setValue", parseInt(val));
		set_hotspot_video_tie(krpano,hotspotName,"height",Math.floor(val*100)/100);
	});
    paster_img_h_Slider.on("slideStop", function (data) {
		$('#paster_img_h_txt').val(data.value);										  
       update_video_tie_value(getCurrScene(),hotspotName,{'height':data.value});
    });

     paster_img_rx_Slider.on("slide", function (data) {
        var krpano=getKrpano();
		$('#paster_img_rx_txt').val(data.value);
        set_hotspot_video_tie(krpano,hotspotName,"rx",data.value);
    });
	 $('#paster_img_rx_txt').bind('input propertychange', function(){
		var krpano=getKrpano();
		var val = $('#paster_img_rx_txt').val();
		if(val<-180) val = -180;
		if(val>180) val = 180;
		paster_img_rx_Slider.slider("setValue", parseInt(val));
		set_hotspot_video_tie(krpano,hotspotName,"rx",Math.floor(val*100)/100);
	});
     paster_img_rx_Slider.on("slideStop", function (data) {
		$('#paster_img_rx_txt').val(data.value);											
       update_video_tie_value(getCurrScene(),hotspotName,{'rx':data.value});
    });

     paster_img_ry_Slider.on("slide", function (data) {
        var krpano=getKrpano();
		$('#paster_img_ry_txt').val(data.value);
        set_hotspot_video_tie(krpano,hotspotName,"ry",data.value);
    });
	 $('#paster_img_ry_txt').bind('input propertychange', function(){
		var krpano=getKrpano();
		var val = $('#paster_img_ry_txt').val();
		if(val<-180) val = -180;
		if(val>180) val = 180;
		paster_img_ry_Slider.slider("setValue", parseInt(val));
		set_hotspot_video_tie(krpano,hotspotName,"ry",Math.floor(val*100)/100);
	});
     paster_img_ry_Slider.on("slideStop", function (data) {
		$('#paster_img_ry_txt').val(data.value);											
       update_video_tie_value(getCurrScene(),hotspotName,{'ry':data.value});
    });

     paster_img_rz_Slider.on("slide", function (data) {
         var krpano=getKrpano();
		 $('#paster_img_rz_txt').val(data.value);
        set_hotspot_video_tie(krpano,hotspotName,"rz",data.value);
    });
	 $('#paster_img_rz_txt').bind('input propertychange', function(){
		var krpano=getKrpano();
		var val = $('#paster_img_rz_txt').val();
		if(val<-180) val = -180;
		if(val>180) val = 180;
		paster_img_rz_Slider.slider("setValue", parseInt(val));
		set_hotspot_video_tie(krpano,hotspotName,"rz",Math.floor(val*100)/100);
	});
     paster_img_rz_Slider.on("slideStop", function (data) {
		$('#paster_img_rz_txt').val(data.value);											
       update_video_tie_value(getCurrScene(),hotspotName,{'rz':data.value});
    });
     
	 paster_img_d_Slider.on("slide", function (data) {
        var krpano=getKrpano();
		$('#paster_img_d_txt').val(data.value);
        set_hotspot_video_tie(krpano,hotspotName,"depth",data.value);
    });
	$('#paster_img_d_txt').bind('input propertychange', function(){
		var krpano=getKrpano();
		var val = $('#paster_img_d_txt').val();
		if(val<200) val = 200;
		if(val>20000) val = 20000;
		paster_img_d_Slider.slider("setValue", parseInt(val));
		set_hotspot_video_tie(krpano,hotspotName,"depth",val);
	});
     paster_img_d_Slider.on("slideStop", function (data) {
		$('#paster_img_d_txt').val(data.value);									   
       update_video_tie_value(getCurrScene(),hotspotName,{'depth': data.value});
    });
    ...
    $("#redpack_name .form-control").keyup(function(e){
        var title = $("#redpack_name input.form-control").val();
        if(title){
            toggleAllNextBtn(true);
        }else{
            toggleAllNextBtn(false);
    ...
    $(document).on('click',"#modal_obj #object_list .col-md-3",function(){
        ...
        if ( type=='obj' ) {
        ...
        }else if(type=='video_tie'){
            html="您已经选择了视频："+$(this).data('name');
            $("#video_choose_ok_tie").data('location',$(this).data('location'));
			$("#video_choose_ok_tie").data('cover',$(this).data('cover'));
            $("#video_choose_ok_tie").data('name',$(this).data('name'));
            $("#video_choose_ok_tie").data('lujin',$(this).data('lujin'));
            $("#video_choose_ok_tie").html(html);
        }
	else if(type == "ring_around"){
            html =' <div class="col-md-3" id="obj_choose_ok" '+
                      '   data-objid="'+$(this).data('ringid')+'" data-thumb="'+$(this).data('img_path')+'"'+
                      '  <div class="card">'+
                      '    <div class="media-wrapper">'+
            ...
            ring_around = 1;
            $("#obj_choose_ok_wrap").html(html);
            // ring_around_id = $(this).data('objid');
        }
	else if(type=='redpack'){
            html =' <div class="col-md-3" id="redpack_choose_ok" data-rpid="'+$(this).data('rpid')+'" data-rpname="'+$(this).data('rpname')+'">'+
                      '  <div class="card">'+
                      '    <div class="media-wrapper">'+
                      '      <div>'+$(this).data("rpname")+'</div>'+ 
                      '     </div>'+
                      '  </div>'+
                      ' </div>';
            $("#redpack_choose_ok_wrap").html(html);
        }
...
function modifySpanClick(el){
    ...
    var check= $('#icon_style .icon_choose input[name=radioOptionsExample][value='+icontype+']').get(0);
    if(check)
        check.checked=true;
        ...
        if(this.name == hotspotName){
            ...
            switch(hotspotidx){
                ...
                case 7:
                    ...
                    break;
                case 8:
                    var hotspotTitle = $('#redpack_name input.form-control').val();
                    var rpid = $('#redpack_choose_ok').data('rpid');
                    var isShowSpotName = $('#redpack_name input[type=checkbox]').is(':checked');
                    param.hotspotTitle = hotspotTitle;
                    param.rpid = rpid;
                    param.rpname =  $('#redpack_choose_ok').data('rpname');
                    param.isShowSpotName = isShowSpotName;
                    hsTitle = param.hotspotTitle;
                    break;    
            }
            ...
            switch(hotspotidx){
                case 0:
					if(param.iconType=="polygon"){
                        setPolygonParams(self,hotSpotDataKey[hotspotidx]);
                        createPolygonHotSpot(krpano,self);
                    }
                    else{	
		    ...
		    break;
				...
				if(param.iconType=="polygon"){
                    	...
                    	krpano.call('addRedpackHotSpot("'+self.imgPath+'","'+ (self.name) +'",'+html_encode(self.hotspotTitle)+','+(self.ath)+','+(self.atv)+','+self.isDynamic+',true,true,'+self.rpid+','+self.isShowSpotName+')');
                    }
		    break;        
            }
...
function initHotSpotDataToPage(hotspotidx,hotspotName){
    ...
    $(hotspotArr).each(function(idx){
        if(this.name == hotspotName){
			...
			}else if(this.iconType == 'polygon'){
                 $('#allAroundModal .icon_choose input[name=radioOptionsExample]:eq(2)').click();
            }
            else{
                $('#allAroundModal .icon_choose input[name=radioOptionsExample]:eq(1)').click();
...
function addRedpackHotSpotFinish(idx){
    var krpano = document.getElementById('panoSettingObject');
    var param = {};
    var hotspotName = getHotSpotName();
    getHotSpotIconData(param);
    ...
    var isShowSpotName = $('#redpack_name input[type=checkbox]').is(':checked');
    var rpid = $('#redpack_choose_ok').data('rpid');
    var rpname = $('#redpack_choose_ok').data('rpname');
    krpano.set("curscreen_x", $('#settingPano').width() / 2);
    krpano.set("curscreen_y", $('#settingPano').height() / 2);
    krpano.call("screentosphere(curscreen_x, curscreen_y, curscreen_ath, curscreen_atv);");
    param.ath = krpano.get("curscreen_ath");
...
function resetRedpackInput(data){
    if(data){
        $('#redpack_name input.form-control').val(data.hotspotTitle);
        $('#redpack_name input[type=checkbox]').prop('checked',data.isShowSpotName);
          var html =' <div class="col-md-3" id="redpack_choose_ok" data-rpid="'+data.rpid+'" data-rpname="'+data.rpname+'">'+
                      '  <div class="card">'+
                      '    <div class="media-wrapper">'+
                      '      <div>'+data.rpname+'</div>'+ 
                      '     </div>'+
        ...
        $('#redpack_name input.form-control').val('');
        $('#redpack_name input[type=checkbox]').prop('checked',false);
        $("#redpack_choose_ok_wrap").html("");
    }
}
...
function toggleMediaTab(mediaType) {
    if (mediaType == 'image') {
        ...
        $('#imgUpload').attr("accept","image/jpeg,image/png,image/gif");
    } else {
...
$(document).ready(function () {
    ...
    $(".pano-setting-container button").click(function () {
        ...
        objM.siblings().animate({right: '-420px'}, 1);
...
function initPage(data) {
    ...
    $("#workcomment").html(worksmain.profile);
...
function initPanoSettingSceneChoose(){
    $('#groupbyul').html('');
    var sceneGroup = $('.group-by').data('scenegroup');
    if(sceneGroup!=undefined && sceneGroup.sceneGroups && sceneGroup.sceneGroups.length > 0){
...
function savePanoSetting() {
    ...
    sb.pushData("imgs", imgsArr);
    //视频贴片
    sb.pushData("video_tie_hotspot", build_video_tie_HotspotSetting());
    sb.submit(function () {

    }, function (data) {
        $('.group-by').data('scenegroup',sceneGroupData);
        if (data.flag) {
            panoConfig.video_tie_hotspot= data.data;
            $.zui.messager.show('操作成功', {placement: 'center', type: 'success', time: '3000', icon: 'check'});
        }else{
            $.zui.messager.show('操作失败:'+data.msg, {placement: 'center', type: 'success', time: '3000', icon: 'check'});
        }
...
function initHotspotSetting(sceneName){
    ...
    //初始化视频热点
    init_video_tie(krpano,sceneName,panoConfig.video_tie_hotspot,true);
    bind_video_tie_tip(sceneName);
    if(hotspotObj){
        $.each(hotspotObj,function(key,value){
            if(key == 'scene'){
                ...
                $(value).each(function(idx){
                    initSceneHotSpot(krpano,this,settingFlag,9);
                });
            }
        });
    }else{
...
function initSceneHotSpot(krpano,param,settingFlag,idx){
    ...
    if(settingFlag){
        ...
        switch(idx){
            case 0:
            	if(param.iconType=="polygon"){
                    setPolygonParams(param,hotSpotDataKey[idx]);
                    createPolygonHotSpot(krpano,param);
                }
                else{
                ...
                mthml = '<div class="edit-item" data-respk="'+param.name+'">' +
                    '<a class="card">' +
                    '<span class="caption">' +
                    '<span class="group-location-icon">定位</span><span class="group-change-icon" data-liindex="0" data-iconType="'+param.iconType+'" data-hotspotidx="0">更换图标</span>&nbsp;&nbsp;<span class="group-rename"  data-iconType="'+param.iconType+'" data-liindex="1" data-hotspotidx="0">场景&nbsp;&nbsp;</span><span class="group-delte">删除</span>' +
                    '</span> </a> ' +
                    '<div class="group-item-title"> ' +
            ...
            case 1:
                ...
                mthml = '<div class="edit-item" data-respk="'+param.name+'">' +
                    '<a class="card">' +
                    '<span class="caption">' +
                    '<span class="group-location-icon">定位</span><span  data-iconType="'+param.iconType+'" class="group-change-icon" data-liindex="0" data-hotspotidx="1">编辑链接</span><span class="group-rename"  data-iconType="'+param.iconType+'" data-liindex="1" data-hotspotidx="1">编辑图标</span><span class="group-delte">删除</span>' +
                    '</span> </a> ' +
                    '<div class="group-item-title"> ' +
            ...
            case 2:
                ...
                mthml = '<div class="edit-item" data-respk="'+param.name+'">' +
                    '<a class="card">' +
                    '<span class="caption">' +
                    '<span class="group-location-icon">定位</span><span class="group-change-icon"  data-iconType="'+param.iconType+'" data-liindex="0" data-hotspotidx="2">编辑标题</span><span class="group-rename"  data-iconType="'+param.iconType+'" data-liindex="1" data-hotspotidx="2">图片</span><span data-iconType="'+param.iconType+'" data-liindex="2" data-hotspotidx="2">图标</span><span class="group-delte">删除</span>' +
                    '</span> </a> ' +
                    '<div class="group-item-title"> ' +
                    '<img class="thumbimg" src="'+param.thumbPath+'"><span>'+html_encode(param.hotspotTitle)+'</span></div> ' +
                    '</div>';
            ...
            case 3:
                ...
                mthml = '<div class="edit-item" data-respk="'+param.name+'">' +
                    '<a class="card">' +
                    '<span class="caption">' +
                    '<span class="group-location-icon">定位</span><span  data-iconType="'+param.iconType+'" class="group-change-icon" data-liindex="0" data-hotspotidx="3">编辑内容</span><span class="group-rename"  data-iconType="'+param.iconType+'" data-liindex="1" data-hotspotidx="3">编辑图标</span><span class="group-delte">删除</span>' +
                    '</span> </a> ' +
                    '<div class="group-item-title"> ' +
                    '<img class="thumbimg" src="'+param.thumbPath+'"><span>'+html_encode(param.hotspotTitle)+'</span></div> ' +
                    '</div>';
            ...
            case 4:
                ...
                mthml = '<div class="edit-item" data-respk="'+param.name+'">' +
                    '<a class="card">' +
                    '<span class="caption">' +
                    '<span class="group-location-icon">定位</span><span  data-iconType="'+param.iconType+'" class="group-change-icon" data-liindex="0" data-hotspotidx="4">编辑标题</span><span data-iconType="'+param.iconType+'" class="group-rename" data-liindex="1" data-hotspotidx="4">音频</span><span data-iconType="'+param.iconType+'" data-liindex="2" data-hotspotidx="4">图标</span><span class="group-delte">删除</span>' +
                    '</span> </a> ' +
                    '<div class="group-item-title"> ' +
                    '<img class="thumbimg" src="'+param.thumbPath+'"><span>'+html_encode(param.hotspotTitle)+'</span></div> ' +
                    '</div>';
            ...
            case 5:
                ...
                mthml = '<div class="edit-item" data-respk="'+param.name+'">' +
                    '<a class="card">' +
                    '<span class="caption">' +
                    '<span class="group-location-icon">定位</span><span data-iconType="'+param.iconType+'" class="group-change-icon" data-liindex="0" data-hotspotidx="5">编辑内容</span><span class="group-rename" data-iconType="'+param.iconType+'" data-liindex="1" data-hotspotidx="5">编辑图标</span><span class="group-delte">删除</span>' +
                    '</span> </a> ' +
                    '<div class="group-item-title"> ' +
                    '<img class="thumbimg" src="'+param.thumbPath+'"><span>'+html_encode(param.hotspotTitle)+'</span></div> ' +
                    '</div>';
            ...
            case 7:
                ...
                mthml = '<div class="edit-item" data-respk="'+param.name+'">' +
                    '<a class="card">' +
                    '<span class="caption">' +
                    '<span class="group-location-icon">定位</span><span data-iconType="'+param.iconType+'" class="group-change-icon" data-liindex="0" data-hotspotidx="7">编辑内容</span><span class="group-change-icon"  data-iconType="'+param.iconType+'" data-liindex="1" data-hotspotidx="7">环物</span><span class="group-rename"  data-iconType="'+param.iconType+'" data-liindex="2" data-hotspotidx="7">图标</span><span class="group-delte">删除</span>' +
                    '</span> </a> ' +
                    '<div class="group-item-title"> ' +
                    '<img class="thumbimg" src="'+param.thumbPath+'"><span>'+html_encode(param.hotspotTitle)+'</span></div> ' +
                    '</div>';
            ...
            case 8:
                ...
                mthml = '<div class="edit-item" data-respk="'+param.name+'">' +
                    '<a class="card">' +
                    '<span class="caption">' +
                    '<span class="group-location-icon">定位</span><span  data-iconType="'+param.iconType+'" class="group-change-icon" data-liindex="0" data-hotspotidx="8">标题</span><span class="group-change-icon" data-iconType="'+param.iconType+'" data-liindex="1" data-hotspotidx="8">视频</span><span class="group-rename" data-iconType="'+param.iconType+'" data-liindex="2" data-hotspotidx="8">图标</span><span class="group-delte">删除</span>' +
                    '</span> </a> ' +
                    '<div class="group-item-title"> ' +
                    '<img class="thumbimg" src="'+param.thumbPath+'"><span>'+html_encode(param.hotspotTitle)+'</span></div> ' +
                    '</div>';
	    ...
	    case 9:
	    		if(param.iconType=="polygon"){
                	...
                	krpano.call('addRedpackHotSpot("'+param.imgPath+'","'+ (param.name) +'",'+html_encode(param.hotspotTitle)+','+(param.ath)+','+(param.atv)+','+param.isDynamic+','+settingFlag+','+isBtnClicked(1)+','+param.rpid+','+param.isShowSpotName+')');
                }
		mthml = '<div class="edit-item" data-respk="'+param.name+'">' +
                    '<a class="card">' +
                    '<span class="caption">' +
                    '<span class="group-location-icon">定位</span><span class="group-change-icon" data-liindex="0" data-hotspotidx="9">标题</span><span class="group-change-icon" data-liindex="1" data-hotspotidx="9">红包</span><span class="group-rename" data-liindex="2" data-hotspotidx="9">图标</span><span class="group-delte">删除</span>' +
                    '</span> </a> ' +
                    '<div class="group-item-title"> ' +
                    '<img class="thumbimg" src="'+param.thumbPath+'"><span>'+html_encode(param.hotspotTitle)+'</span></div> ' +
                    '</div>';
	    ...
	    case 9:
	    		if(param.iconType=="polygon"){
                	...
                	krpano.call('addObjHotSpot("'+param.imgPath+'","'+ (param.name) +'",'+html_encode(param.hotspotTitle)+','+(param.ath)+','+(param.atv)+','+param.isDynamic+','+settingFlag+',true,'+param.rpid+','+param.isShowSpotName+')');
                }
		break;
        }
    }
...
function choose_redpack(){
    $.post('/member/redpack',{
        ...
        for(var i =0 ; i<res.length ; i++){
            var o = res[i];
            html+=' <div class="col-md-3" data-rpname="'+o.name+'" data-rpid="'+o.id+'">'+
                  '  <div class="card">'+
                  '    <div class="card-heading">'+
                  '         <span class="card-scene-name">'+o.name+'</span>'+
                  '     </div>'+
        ...
        $('#modal_obj .modal-header h3').html('请选择红包');
        $('#modal_obj #object_list').html(html);
    },'json');
}
function choose_videos(media_type){

    var sb = _U.getSubmit("/member/mediares", null, "ajax", false);
    sb.pushData("act", 'list');
    sb.pushData("type", 'custom');
    sb.pushData("media_type", media_type);
    sb.submit(function () {
       ...
       for(var i =0 ; i<res.length ; i++){
           var o = res[i];
           html+=' <div class="col-md-3" data-name="'+o.media_name+'" data-id="'+o.pk_media_res+'" data-cover="'+o.cover+'" data-lujin="'+o.media_path+'" data-location="'+o.absolutelocation+'">'+
                 '  <div class="card">'+
                 '    <div class="media-wrapper" style="height: 130px;padding-top: 26px;background-image: url('+o.cover+');">'+
...
function choose_ring(){
    $.post('/member/ring_around',{
        ...
        for(var i =0 ; i<res.length ; i++){
            var o = res[i];
            html+=' <div class="col-md-3" data-rname="'+o.rname+'" data-ringid="'+o.id+'" data-img_path="'+o.img_path+'">'+
                  '  <div class="card">'+
                  '    <div class="media-wrapper">'+
                  '      <img src="'+o.img_path+'" style="height:120px">'+
                  '     </div>'+
                  '    <div class="card-heading">'+
                  '         <span class="card-scene-name">'+o.rname+'</span>'+
                  '     </div>'+
                  '  </div>'+
                  ' </div>';
        }
        $('#modal_obj .modal-header h3').html('请选择720环物');
        $('#modal_obj #object_list').html(html);
    },'json');
}
//截断缩略图后边的问号，获得原图
...
function choose_pasterimg(){
    chooseMediaResCallBack = choose_pasterimg_ok;
}
function choose_pasterimg_ok(mediaData){
   var krpano = getKrpano();
   var sceneName = krpano.get('xml.scene');
    ...
    ath = krpano.get("curscreen_ath");
    atv = krpano.get("curscreen_atv");
    krpano.call('addimg_tie_hotspot("'+mediaData.src+'","'+hotspotName+'","'+mediaData.title+'","'+ath+'","'+atv+'",0,0,0,0,0,true,200,true,20,20,true)');
    
    var hotspot = krpano.get("hotspot['"+hotspotName+"']");
    var obj = get_video_tie_scene(sceneName);
    var scene = recordhotspot(hotspot,'',sceneName,'img');
    if (obj==null) {
