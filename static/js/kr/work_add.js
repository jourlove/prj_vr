...
$(function(){
	...
	$("#imgUpload").fileinput({
		...
		layoutTemplates: {
            ...
            '       {upload}\n' +
            '       {browse}\n' +
            '<div style="margin-left:10px;display:inline-block;" id="addMaterialButtonDiv"><button class="btn" type="button" onclick="addSceneClick(this)">选择素材库全景图片 </button> </div>' +
            '   </div>\n' +
            '</div>'
        }
...
function submitMaterial(){
	 var len=$("#myContainer div img").length;
...
function commit(){
	...
	params.imgs=new Array();
	 ...
	 params.mimgs=new Array();
	 //素材库图片
	 $("#myContainer div img").each(function(i,item){
	 	var img={};
	 	// img.imgsrc = $(this).data("src");
	 	// img.imgname = $(this).data("title");
	 	img.pk_img_main=$(item).attr('data-id');
	 	params.mimgs.push(img);
	 });
...
function bindImgcallback(data){
    addPicture(data);
}
function addPicture(checkedArr) {
    var html = '';
    for (var i = 0; i < checkedArr.length; i++) {
        var imgObj = checkedArr[i];
        var $myContainer= $("#pranoimg #myContainer");
        var cnt=$myContainer.find('>div[id='+imgObj.pk_img_main+']').length;
        if(cnt==0){
            html += '<div id="'+imgObj.pk_img_main+'" class="file-preview-frame" data-fileindex="'+i+'" data-template="image">'+
            '<div class="kv-file-content">'+
            '<img class="file-preview-image" style="width:auto;height:100px;" data-view_uuid="'+imgObj.view_uuid+'" data-id="'+imgObj.pk_img_main+'" alt="'+imgObj.filename+'"' +
            ' title="'+imgObj.filename+'" src="'+imgObj.location+'">'+
            '</div>'+
            '<div class="file-thumbnail-footer">'+
            '<div class="file-footer-caption" title="'+imgObj.filename+'">'+imgObj.filename+'<br><samp>（素材库）</samp></div>'+
            '<div class="file-thumb-progress hide">'+
            '<div class="progress">'+
            '<div class="progress-bar progress-bar-success progress-bar-striped active" style="width:0%;" aria-valuemax="100" aria-valuemin="0" aria-valuenow="0" role="progressbar"> 0% </div>'+
            '</div>'+
            ...
            '<div class="file-actions">'+
            '<div class="file-footer-buttons">'+
            '<button class="kv-file-remove1 btn btn-xs btn-default" title="删除文件" onclick="delpic('+imgObj.pk_img_main+')" type="button">'+
            '<i class="icon icon-trash text-danger"></i>'+
            '</button>'+
            // '<button type="button" class="kv-file-zoom btn btn-xs btn-default" title="View Details"><i class="icon icon-zoom-in"></i></button>'+
            '</div>'+
            '<div class="file-upload-indicator" title="没有上传"><i class="icon icon-hand-down text-warning"></i></div>'+
            '<div class="clearfix"></div>'+
            '</div>'+
            '</div>'+
            '</div>';
         ...
         alert_notice(imgObj.filename+"已经存在与选中列表里");
        }
    }
     if ($myContainer.length == 0) {
        $("#pranoimg .file-preview").find(".file-drop-zone-title").remove();
        $("#pranoimg .file-preview").find(".file-preview-thumbnails").before("<div id = 'myContainer'></div>");
        $myContainer=$("#pranoimg #myContainer");
    }
    $myContainer.append(html);
}
function delpic(id) {
   var $myContainer= $("#pranoimg #myContainer");
   $myContainer.find('>div[id='+id+']').remove();
    var files = $('#imgUpload').fileinput('getFileStack');
   if($myContainer.children().length<=0 && files.length<=0){
        $("#pranoimg .file-preview").find(".file-preview-thumbnails").before("<div class='file-drop-zone-title'>拖拽一组/单幅图片或点击下面按钮上传</div>");
        $myContainer.remove();
   }
}
//截断缩略图后边的问号，获得原图
