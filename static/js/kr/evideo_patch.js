...
$(function(){
    ...
    if(typeof(tag)!="undefined" && tag!=null && tag==1){
       ...
       get_video_tie_hotspotByName(sceneName,name,function(scene,hots,index){
        ...
        krpano.call('addvideo_tie_hotspot("'+loc+'","'+hotspotName+'","'+name+'","'+ath+'","'+atv+'",0,0,0,0,0,true,200,true,20,20,true)');
        
        var hotspot = krpano.get("hotspot['"+hotspotName+"']");
		if(hotspot!=null){
        var scene=recordhotspot(hotspot,sceneName);
        if(obj==null){
    ...
    $.each(scene,function(i,s){
        if(s.name==sceneName){
            $.each(s.hotspot,function(j,h){
                ...
                krpano.call('addvideo_tie_hotspot("'+h.videourl+'","'+h.name+'","'+h.hotspottitle+'","'+h.ath+'","'+h.atv+'","'+h.rx+'","'+h.ry+'","'+h.rz+'",0,0,true,"'+h.depth+'",true,"'+h.width+'","'+h.height+'","'+flag+'")');
		        }else{
                  krpano.call('addvideo_tie_hotspot("'+h.videourl+'","'+h.name+'","'+h.hotspottitle+'","'+h.ath+'","'+h.atv+'","'+h.rx+'","'+h.ry+'","'+h.rz+'",0,0,true,"'+h.depth+'",true,"'+h.width+'","'+h.height+'","'+flag+'","'+h.posterurl+'","'+h.locationurl+'")');
                }
            });
            return false;
        }
...
function bind_video_tie_tip(sceneName){
    ...
    var $tbody=$(".tie .tab-content .tab_footer table tbody");
     $tbody.html('');
   ...
   if(obj!=null){
      $.each(obj.hotspot,function(i,item){
          ...
          var len=$(".tie .tab-content .tab_footer table tbody tr[id="+hotspotName+"]").length;
          if(len<=0){
              ...
              '</td></tr>';
             $tbody.append(str);
             var $a=$(".tie .tab-content .tab_footer table tbody tr[id="+hotspotName+"] td a")
             $a.data('hot',item);
      ...
      //设置radio
      var $radio=$(".tie .tab-content .tab_footer table tbody tr:last input[type='radio']");
      if($radio.get(0)){
...
* 构建视频贴片数据格式
*/
function recordhotspot(hs,sceneName){
    var scene={};
...
*@hotspotName 热点名称
*/
function set_Slider(sceneName,hotspotName){
...
function create_video_tie_hotspot(krpano,hotsopt_config,flag){
  ...
  krpano.call('addvideo_tie_hotspot("'+h.videourl+'","'+h.name+'","'+h.hotspottitle+'","'+h.ath+'","'+h.atv+'","'+h.rx+'","'+h.ry+'","'+h.rz+'","'+h.ox+'","'+h.oy+'",true,"'+h.depth+'",true,"'+h.width+'","'+h.height+'","'+flag+'")');
}
