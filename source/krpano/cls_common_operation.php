...
abstract class KrOperation{
	...
	*/
	public static function slice($pk_user_main,$imgs,$origin_dir){
		...
		switch ($GLOBALS['_lang']['global_storage']) {
			...
			case 'oss':
				...
				break;
			case "local":
				require_once __DIR__.'/../local/cls_local.php';
				$kr=new Local();
				$cdn_host .= $GLOBALS['_lang']['local_config']['cdn_host'];
				break;
		}
		$cdn_host .='/';
	...
	public static function get_video_thumb($location,$time,$thumbpath){
		$kr = null;
		...
		return $kr==null?null:$kr->video_thumb($location,$time,$thumbpath);
	}
	...
	private function slicing($imgs,$temp_dir,$cdn_host,$origin_dir){
		...
		if ($path!="") {
			...
			//执行切图
			if($GLOBALS['_lang']['multi_pano']==1){
				exec(KRPANO_MULTI." ".$path."");
			}else{
				exec(KRPANO." ".$path."");
			}
			//上传切好图的整个目录到服务器
		...
		$result = array('imgsmain'=>$imgsmain,'temp_dir'=>$temp_dir);
		return  $result;
		//return  $imgsmain;
