...
class Transaction extends Common{
	...
	public static function get_site_config(){
		...
		foreach($conf as $k=>$v){
			...
			//if($v['name']=='qiniu')	$v['value'] = 'disable';
			//if($v['name']=='local')	$v['value'] = 'enable';
			...
			if($v['value']=='enable' || $v['value']=='disable'){
				...
				//组装qiniu参数
				if($v['name']=='qiniu'){
					$GLOBALS['_lang']['qiniu_config']['up_url'] = 
					$GLOBALS['_lang']['qiniu_zone'][$GLOBALS['_lang']['qiniu_config']['zone']]['up_url'];
				}
				//组装oss参数
				if($v['name']=='oss'){
					$GLOBALS['_lang']['oss_config']['external_url'] = 
						$GLOBALS['_lang']['oss_zone'][$GLOBALS['_lang']['oss_config']['zone']]['external_url'];
					$GLOBALS['_lang']['oss_config']['internal_url'] = 
						$GLOBALS['_lang']['oss_zone'][$GLOBALS['_lang']['oss_config']['zone']]['internal_url'];
					$GLOBALS['_lang']['oss_config']['location'] = 
						$GLOBALS['_lang']['oss_zone'][$GLOBALS['_lang']['oss_config']['zone']]['location'];
				    //如果不使用内网，内网赋值为外网
		...
		switch ($GLOBALS['_lang']['global_storage']) {
			case 'qiniu':
				...
				break;
			case 'oss':
				$GLOBALS['_lang']['up_url'] = 'http://'.$GLOBALS['_lang']['oss_config']['bucket'].'.'.
				                              $GLOBALS['_lang']['oss_config']['external_url'];
				break;
			case 'local':
				$GLOBALS['_lang']['up_url'] ='/source/local/upload_local.php';
				break;
		}
	...
	private static $_storage;
   	public static function get_storage($global_storage)
   	{
      $global_storage = !$global_storage ? $GLOBALS['_lang']['global_storage'] : $global_storage;
      if(!Transaction::$_storage){
         if($global_storage == 'qiniu'){
            require_once ROOT_PATH.'source/qiniu/cls_qiniu.php';
            Transaction::$_storage = new Qiniu();
         }
         else if($global_storage == 'oss'){
            require_once ROOT_PATH.'source/oss/cls_oss.php';
            Transaction::$_storage = new Oss();
         }
		 else if($global_storage == 'local'){
            require_once ROOT_PATH.'source/local/cls_local.php';
            Transaction::$_storage = new Local();
         }
      }
      return Transaction::$_storage;
   	}
