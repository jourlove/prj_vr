...
if($act =="update_init"){
	...
	if ($Db->getCount($Base->table('worksmain'),"pk_works_main",array("pk_works_main"=>$pk_works_main ,"pk_user_main"=>$user['pk_user_main']))<=0) {
		...
		$params = array(
			...
			'tour_guide'=>$Json->encode_unescaped_unicode($input['tour_guide']),
			'scene_group'=>$Json->encode_unescaped_unicode($input['scene_group']),
			'video_tie_hotspot'=>$Json->encode($input['video_tie_hotspot'])
			 );
		$Db->update($Base->table('pano_config'),$params,array("pk_works_main"=>$pk_works_main));

		$video_tie_hotspot=$input['video_tie_hotspot'];
		$result['flag'] = 1;
		$result['data']=$video_tie_hotspot;
	}
	...
	if (empty($works)||empty($panoconfig)) {
		...
		try{
			...
			$panoconfig_params = array(
					...
					'reward' => empty($panoconfig['reward'])?0:intval($panoconfig['reward']),
					...
					'vcard' => $Json->encode_unescaped_unicode($panoconfig['vcard']),
					'loading_img' => $Json->encode_unescaped_unicode($panoconfig['loading_img']),
